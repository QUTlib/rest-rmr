...
tr:nth-child(odd){background-color:#d8d8d8}
...
<body>
<h1><a href="/debug/">Debug</a>: Handlers</h1>
<table>
<tr><th>Method</th><th>Pattern</th><th>Handler</th></tr>
HTML;
	foreach (self::$map as $m=>$a) {
		echo '<tr class="big">
<th rowspan="' . count($a) . '">' . $m . '</th>
';
		$first =true;
		foreach ($a as $x) {
			if ($first)
			...
			if (is_array($h)){
				if(count($h)>2){
					...
					echo '<td><code>('.htmlspecialchars(get_class($h[0])).')-&gt;'.htmlspecialchars($h[1])."()</code></td>\n";
				}
			}else{
				echo '<td><code>' . htmlspecialchars($h)."()</code></td>\n";
			}
		echo "</tr>\n";
		}
	}
